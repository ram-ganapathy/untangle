  ...
  return (
    <main className="page home-page">
      <div className="shell">
        ...
        {spirals.length > 0 && <section className="memory-shelf" aria-label="Your saved spirals">{spirals.map((spiral) => <article className="memory-vial-card" key={spiral.id}><a className="memory-vial-link" href={`#/spiral/${spiral.id}`} aria-label={`${spiral.title}, ${spiral.diagnosis || 'waiting'}, ${spiral.state}`}><MemoryVial diagnosis={spiral.diagnosis} state={spiral.state} /><span className="memory-vial-label"><strong>{spiral.title}</strong><small>{touchedAt(spiral.updatedAt)}</small></span></a><span className="memory-vial-erase"><EraseSpiral spiralId={spiral.id} onErased={removeSpiral} /></span></article>)}</section>}
      </div>
