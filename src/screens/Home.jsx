...
function MemoryVial({ diagnosis, state, safety }) {
  const kind = safety ? 'care' : ['replay', 'projection', 'rumination', 'deliberation'].includes(diagnosis) ? diagnosis : 'waiting'
  const stateClass = safety ? 'vial-care' : state === 'settled' ? 'vial-settled' : 'vial-open'
  return <span className={`memory-vial vial-${kind} ${stateClass}`} aria-hidden="true"><span className="vial-cork" /><span className="vial-glass"><span className="vial-content"><i /><i /><i /></span></span></span>
}
  ...
  return (
    <main className="page home-page">
      <div className="shell">
        ...
        {spirals.length > 0 && <><section className="memory-shelf" aria-label="Your saved spirals">{spirals.map((spiral) => { const label = spiral.safety ? 'Held with care' : spiral.title; return <article className="memory-vial-card" key={spiral.id}><a className="memory-vial-link" href={`#/spiral/${spiral.id}`} aria-label={`${label}, ${spiral.safety ? 'care' : spiral.diagnosis || 'waiting'}, ${spiral.state}`}><MemoryVial diagnosis={spiral.diagnosis} state={spiral.state} safety={spiral.safety} /><span className="memory-vial-label"><strong>{label}</strong><small>{touchedAt(spiral.updatedAt)}</small></span></a></article> })}</section><a className="look-up-link" href="#/sky">Look up</a></>}
      </div>
